-- TransitOps schema, triggers, RLS, and seed data
...
-- Trip status validation and auto-transitions
...
-- Maintenance status flips vehicle status
...
begin
  ...
  values (
    ...
    coalesce((new.raw_user_meta_data->>'role')::app_role, 'fleet_manager')
  )
...
declare t text;
begin
  foreach t in array array['vehicles','drivers','trips','maintenance_logs','fuel_logs','expenses']
  loop
